...
export default function OwnerOtpScreen() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      Alert.alert('رمز غير صحيح', err?.message ?? err?.response?.data?.message ?? 'يرجى التحقق من الرمز');
      setOtp('');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={['#0A0E1A', '#0D1535', '#0A0E1A']} style={StyleSheet.absoluteFill} />
      <SafeAreaView style={styles.safe}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}>
          <Text style={[Typography.bodyLg, { color: Colors.text.secondary }]}>← رجوع</Text>
        </TouchableOpacity>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.content}>
          <Text style={[Typography.h1, { color: Colors.text.primary, textAlign: 'center' }]}>رمز التحقق</Text>
          <Text style={[Typography.bodyMd, { color: Colors.text.secondary, textAlign: 'center', lineHeight: 24 }]}>
          ...
          <TouchableOpacity onPress={() => inputRef.current?.focus()} activeOpacity={1} style={styles.digitRow}>
            {Array.from({ length: OTP_LENGTH }).map((_, i) => (
              ...
              >
                <Text style={[Typography.h2, { color: Colors.text.primary }]}>{digits[i] ?? ''}</Text>
              </View>
            ))}
          </TouchableOpacity>
          ...
          <PrimaryButton label="تحقق" onPress={handleVerify} loading={loading} disabled={otp.length < OTP_LENGTH} />

          <TouchableOpacity
            onPress={async () => {
              if (countdown > 0) return;
              try { await sendOtp(phone); setCountdown(RESEND_COOLDOWN); setOtp(''); }
              catch { Alert.alert('خطأ', 'تعذّر إعادة الإرسال'); }
            }}
...
const styles = StyleSheet.create({
  ...
  digitBoxFilled: { borderColor: Colors.brand.primary, backgroundColor: Colors.brand.primary + '15' },
  digitBoxActive: {
